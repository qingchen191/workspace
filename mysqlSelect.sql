...
SELECT hour(addtime),count(*) FROM qingchen_171.wqtab where game='PopStar' and addtime > '2013-06-30' and addtime < '2013-07-01' and type = 'view' group by hour(addtime);
...
(select game,count(*) cnt from wqtab where type = 'view' and date_format(addtime,'%Y-%m-%d') = '2013-07-01' group by game) v,
(select game,count(*) cnt,count(*)*0.3 amount from wqtab where type = 'click' and date_format(addtime,'%Y-%m-%d') = '2013-07-01' group by game) c,
wqgames g
