...
SELECT hour(addtime),count(*) FROM qingchen_171.wqtab where game='%E4%BF%9D%E5%8D%AB%E9%92%93%E9%B1%BC%E5%B2%9B' and date_format(addtime,'%Y-%m-%d') = '2013-07-03' and type = 'click' group by hour(addtime);
...
(select game,count(*) cnt from wqtab where type = 'view' and date_format(addtime,'%Y-%m-%d') = '2013-07-04' group by game) v,
(select game,count(*) cnt,count(*)*0.3 amount from wqtab where type = 'click' and date_format(addtime,'%Y-%m-%d') = '2013-07-04' group by game) c,
wqgames g
